...
1. ADDS the ``sum`` / ``mean`` *reduction* ops (with correct backward) to the
   ``Tensor``.  stage_09 deliberately shipped no reductions and deferred them to
...
# stage_12 when it exists (stage_12 adds broadcasting-correct backward and may
# export ``class Tensor(Stage9_Tensor)``); otherwise we fall back to the raw
# stage_09 Tensor.  Either way, THIS stage subclasses the base to add reductions.
from dlfs import stage_import

Stage9_Tensor = stage_import("stage_09", "Tensor")
try:
    # Prefer stage_12's broadcasting Tensor so reductions sit on top of it.
    _Base = stage_import("stage_12", "Tensor")
except ImportError:
    # stage_12 has not exported a Tensor subclass yet -> build on stage_09.
    _Base = Stage9_Tensor
...
class Tensor(_Base):
    ...
    Subclasses the broadcast-capable base (stage_12's ``Tensor`` if present, else
    stage_09's) and ADDS the two reduction ops the losses need.  A reduction maps

...
* ``MLP`` -- a multilayer perceptron: a chain of pure-linear ``Dense`` layers
  (stage_11) with a nonlinearity applied between them, built on the autodiff
  ``Tensor`` (stage_09).
* ``Tensor`` -- a thin subclass of the stage_09 ``Tensor`` that finally adds the
  **broadcasting-correct backward** stage_09 deferred to here.  The stage_09
  ...
  (the "unbroadcast" rule) back to that parent's original shape.  stage_13's
...
# number/array we wrap (mirrors stage_09's ``Operand`` alias).
Operand = Union["Tensor", float, int, np.ndarray, list]


class Tensor(Stage9_Tensor):
    """The stage_09 autodiff ``Tensor`` extended with broadcasting backward.

    stage_09 keeps elementwise binary ops to equal-shaped operands and defers
    "general broadcasting gradient reduction to stage_12" -- that is this class.
    ...
    ``log``, ``@``) are inherited unchanged from stage_09; the derived ops
    ...
    def __add__(self, other: "Operand") -> "Tensor":
        ...
        reduce to the stage_09 behaviour (unbroadcast is then a no-op)."""
...
# stage_13's stable softmax needs ``(B,C) - (B,1)`` to backprop correctly; that
# broadcasting backward is delivered HERE, satisfying stage_13's reliance even
# though stage_13 keeps importing the engine via ``stage_import("stage_09",

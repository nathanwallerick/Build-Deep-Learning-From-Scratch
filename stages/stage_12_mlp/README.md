...
**Cumulative framework** — This stage imports `Tensor` (`stage_09`) and `Dense` (`stage_11`) via `dlfs.stage_import` and **adds** the new `MLP` class that composes `Dense` layers + activations on top of them (no redefinition of `Dense`). It *also* adds a broadcasting-aware `Tensor` subclass: `class Tensor(Stage9_Tensor)` overrides the elementwise binary ops (`__add__`/`__mul__`, and the derived/reflected forms that compose out of them) so that differently-shaped-but-broadcastable operands forward via NumPy broadcasting *and* backprop correctly — each parent's gradient is reduced ("unbroadcast") back to its own shape. This is exactly the "general broadcasting gradient reduction" that `stage_09` promised to defer to here, and `stage_13`'s stable softmax (`logits - logsumexp(..., keepdims=True)`, i.e. `(B,C) - (B,1)`, and bias-row broadcasting) relies on it.
